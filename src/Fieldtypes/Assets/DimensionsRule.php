<?php

namespace Statamic\Fieldtypes\Assets;

use Closure;
use Illuminate\Contracts\Validation\ValidationRule;
use Statamic\Contracts\GraphQL\CastableToValidationString;
use Statamic\Facades\Asset;
use Statamic\Statamic;
use Stringable;
use Symfony\Component\HttpFoundation\File\UploadedFile;

class DimensionsRule implements CastableToValidationString, Stringable, ValidationRule
{
    protected $string_parameters;
    protected $width;
    protected $height; 

    public function __construct(protected $parameters)
    {
        $this->string_parameters = $parameters;
        $this->parameters = array_reduce($parameters, function ($result, $item) {
            [$key, $value] = array_pad(explode('=', $item, 2), 2, null);

            $result[$key] = $value;

            return $result;
        });
    }

    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        $size = [0, 0];

        if ($value instanceof UploadedFile) {
            if (in_array($value->getMimeType(), ['image/svg+xml', 'image/svg'])) {
                return;
            }

            $size = getimagesize($value->getPathname());
        } elseif ($asset = Asset::find($value)) {
            if ($asset->isSvg()) {
                return;
            }

            $size = $asset->dimensions();
        }

        [$width, $height] = $size;
        if (! is_int($width) || ! is_int($height)) {
            $fail(__('statamic::validation.dimensions.unknown'));

            return;
        }

        if ($message = $this->message($width, $height)) {
            $fail($message);
        }
    }

    public function message(int $width, int $height): ?string
    {
        $valid_ratio = $this->validateRatio($width, $height);
        $valid_width = $this->validateWidth($width);
        $valid_height = $this->validateHeight($height);

        if ($valid_ratio && $valid_width && $valid_height) {
            return null;
        }

        $prefix = (Statamic::isCpRoute() ? 'statamic::' : '') . 'validation.dimensions';

        return __(
            "{$prefix}.invalid",
            [
                'dimensions' => collect($this->parameters)
                    ->map(fn ($value, $key) => __("{$prefix}.{$key}") . ' = ' . $value)
                    ->join(', ', ' and ')
            ],
        );
    }

    public function validateWidth(int $width): bool
    {
        $valid = true;

        if (isset($this->parameters['width']) && $this->parameters['width'] != $width) {
            $valid = false;
        }

        if (isset($this->parameters['min_width']) && $this->parameters['min_width'] > $width) {
            $valid = false;
        }

        if (isset($this->parameters['max_width']) && $this->parameters['max_width'] < $width) {
            $valid = false;
        }

        return $valid;
    }

    public function validateHeight(int $height): bool
    {
        $valid = true;

        if (isset($this->parameters['height']) && $this->parameters['height'] != $height) {
            $valid = false;
        }

        if (isset($this->parameters['min_height']) && $this->parameters['min_height'] > $height) {
            $valid = false;
        }

        if (isset($this->parameters['max_height']) && $this->parameters['max_height'] < $height) {
            $valid = false;
        }

        return $valid;
    }

    public function validateRatio(int $width, int $height): bool
    {
        if (! isset($this->parameters['ratio'])) {
            return true;
        }

        [$numerator, $denominator] = array_replace(
            [1, 1],
            array_filter(sscanf($this->parameters['ratio'], '%f/%d'))
        );

        $precision = 1 / (max($width, $height) + 1);

        return abs($numerator / $denominator - $width / $height) > $precision;
    }

    public function __toString()
    {
        return 'dimensions:'.implode(',', $this->string_parameters);
    }

    public function toGqlValidationString(): string
    {
        return $this->__toString();
    }
}
